...
function khaiIMS_IgateO40(card, pon, id) {
...
$(document).ready(function () {
    ...
    $('#btn-createAll').click(function () {
        ...
        var pon = getPon(SLID);
        var id = getID(SLID);
        var idPassword = $('#idPassword').val().replace(/00/, '/').replace(/0/g, '/');

        if (typeOLT.length > 0 || typeModem.length > 0) {
            if (typeModem.text() == 'ALU-240') {
                var chuoi = XoaONU_OLT_ALU(card, pon, id) + "\r\n" +
                    khaiONU_240_OLT_ALU(card, pon, id) + "\r\n" +
                    khaiNet_240_OLT_ALU(card, pon, id, vlanNet, vlanGNMS) + "\r\n"
                khaiMyTV_240(card, pon, id, vlanMytv);
                $('#outputCLI').html(chuoi);
            }
            else if (typeModem.text() == 'Vlink1000') {
                var chuoi = XoaONU_OLT_ALU(card, pon, id) + "\r\n" +
    ...
    $('#btn-create-onu').click(function () {
        ...
        var pon = getPon(SLID);
        var id = getID(SLID);
        var chuoi = khaiONU_OLT_ALU(card, pon, id)
        var idPassword = $('#idPassword').val().replace(/00/, '/').replace(/0/g, '/');
        if (typeOLT.length > 0 || typeModem.length > 0) {
    ...
    $('#btn-net').click(function () {
        ...
        if (typeOLT.length > 0 || typeModem.length > 0) {
            ...
            if (typeModem.text() == 'ALU-240') {
                $('#outputCLI').html(khaiNet_240_OLT_ALU(card, pon, id, vlanNet, vlanGNMS));
            }
            else if (typeModem.text() == 'Vlink1000') {
                $('#outputCLI').html(khaiNetVlan1000_OLT_ALU(card, pon, id, vlanNet, vlanGNMS));
            }

            //cac loai modem igate 
            else {
                $('#outputCLI').html(khaiNet_OLT_ALU(card, pon, id, vlanNet, vlanGNMS));
            }
        };
    });
    //Khai MyTV
    $('#btn-mytv').click(function () {
        ...
        var idPassword = $('#idPassword').val().replace(/00/, '/').replace(/0/g, '/');
        if (typeOLT.length > 0 || typeModem.length > 0) {
            if (typeModem.text() == 'Vlink1000') {
                // $('#outputCLI').html( khaiMyTV_240(card,pon,id,vlanMytv));
            }
            else if (typeModem.text() == 'ALU-240') {
                $('#outputCLI').html(khaiMyTV_240(card, pon, id, vlanMytv));
            }
            else {
